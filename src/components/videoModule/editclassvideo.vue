    ...
    <section >
      <el-form ref="ruleForm" :model="ruleForm" label-width="120px">
        ...
        <el-form-item label="分类名称" prop="name">
          <el-input style="width: 300px" :maxlength="4" v-model="ruleForm.name"></el-input>
        </el-form-item>
...
<script>
  export default {
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$axios.post("/video_group/update",databesedit).then((response) =>{
    ...
    created(){
      this.$axios.post('/video_group/read',{id:this.$route.query.data}).then((response)=>{
        if(response.data.status === 200){
