      ...
      <el-table
        ...
        <el-table-column label="分类图片">
          <template slot-scope="scope">
            <img :src="scope.row.cover_picture" alt="" style="width: 50px;height: 50px">
          </template>
        </el-table-column>
        <el-table-column prop="type" label="父级分类">
...
<script>
  export default {
    ...
    methods:{
      ...
      sendSortValue(){
        this.$axios.post('/video_group/index',{name:this.searchValue}).then((response)=>{
      ...
      handleCurrentChanges(val){
        ...
        this.$axios.post('/video_group/index',{page:val}).then((response) =>{
      ...
      handleEdit(index, row) {
        this.$router.push({name:'editclassvideo',query:{data:row.id}})
      },
      handleDelete(index, row) {
        this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          this.$axios.post('/video_group/delete',{'id':row.id}).then((response) =>{
            if(response.data.status === 200){
              this.$axios.post('/video_group/index',{page:this.currentPage}).then((response) =>{
                if(response.status === 200){
                  this.tableData = response.data.data.data
                  this.sum = response.data.data.total
                }else {
    ...
    created(){
      this.$axios.post('/video_group/index').then((response)=>{
        if(response.data.status === 200){
          this.tableData = response.data.data.data
          this.sum = response.data.data.total
        }else {
