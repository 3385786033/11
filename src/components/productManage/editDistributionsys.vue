                ...
                <div  class="buyBambino-main">
                  <el-form ref="forms" :model="forms" label-width="120px" label-position="left">
                    <el-form-item
                      ...
                      <template>
                        <el-input v-model="forms[item]" style="width: 100px"></el-input><span style="color: red;margin-left: 20px;font-size: 12px"></span>
                      </template>
                    </el-form-item>
                    <el-form-item>
      ...
      this.$axios.post('/product/getProductDetail',{'product_id':this.$route.query.product_id}).then((response)=> {
        if (response.data.status === 1) {
          ...
          if(response.data.data.tableData.length === 0){
            ...
            this.cols = [
              {prop: '规格', label: '规格'},
              {prop: 'option_id', label: 'option_id'},
            ]
            this.dynamicTags = ['规格','option_id']
          }else {
            ...
            this.ruleForm.stock = stor
            //价格
            let min = null
            let max = null
            let arrPirc = []
            for(let i in response.data.data.tableData){
              arrPirc.push(parseInt(response.data.data.tableData[i].价格))
