  ...
  <div class="setegGfrenzy">
    ...
    <section class="setegGfrenzy-main">
      <div class="addVip-mainLeft">
        <div class="img-left">
          <img src="../../assets/images/dzp.jpg" alt="">
        </div>
      </div>
      <div class="addVip-mainRight">
        <el-form ref="form" :model="form" :rules="rules">
          ...
          <el-form-item label="中奖总概率" prop="win_prob">
            <el-input type="number" v-model="form.win_prob" style="width: 80px"></el-input>%
          </el-form-item>
          <el-form-item>
          ...
          <el-form-item label="* 奖品设置">
            <el-button type="primary" @click="addPrize" :disabled="Prize">添加奖品</el-button><i class="s_i">注：最多添加10条</i>
              ...
              <el-table-column label="图标">
                <template slot-scope="scope">
                  <el-upload
                    ...
                    <div slot="tip" class="el-upload__tip">图片大小不超过2M!</div>
                  </el-upload>
                </template>
              </el-table-column>
    ...
    methods:{
      ...
      Rule_handleEdit(index,row){
        if(row.id ===''||row.id === null|| row.id===undefined){
          this.$axios.post('/slyder_adventures/saveRuleExplain',{content:row.content}).then((response)=>{
            if(response.data.status === 200){
              this.$message({
                type: 'success',
                message: '保存成功！'
              });
              this.$axios.post('/slyder_adventures/indexRuleExplain').then((response)=>{
                if(response.data.status === 200){
          ...
          this.$axios.post('/slyder_adventures/updateRuleExplain',{id:row.id,content:row.content}).then((response)=>{
            if(response.data.status === 200){
              this.$message({
                type: 'success',
                message: '保存成功！'
      ...
      Rule_handleDelete(index,row){
        if(this.Rule_tableData.length <= 1){
          ...
          if(row.id ===''||row.id === null|| row.id===undefined){
            ...
            this.$axios.post('/slyder_adventures/deleteRuleExplain',{id:row.id}).then((response)=>{
              if(response.data.status === 200){
                this.$message({
                  type: 'success',
                  message: '删除成功！'
      ...
      prize_changs(row){
        switch(parseInt(row.type)){
          ...
          case 1:
            ...
            this.$axios.post('/slyder_adventures/getVirtualPrize').then((response)=>{
              if(response.data.status === 200){
          ...
          case 2:
            this.isPrize_type = true
            this.$axios.post('/slyder_adventures/getThinks').then((response)=>{
              if(response.data.status === 200){
      ...
      prize_handleEdit(index,row){
        ...
        if(row.id === ''||row.id===null||row.id === undefined){
          ...
          this.$axios.post('/slyder_adventures_prize/save',sendData).then((response)=>{
            if(response.data.status === 200){
              this.$message({
                type: 'success',
                message: response.data.message
              });
              this.$axios.post('/slyder_adventures_prize/index').then((response)=>{
                if(response.data.status === 200){
          ...
          this.$axios.post('/slyder_adventures_prize/update',sendData).then((response)=>{
            if(response.data.status === 200){
              this.$message({
                type: 'success',
                message: response.data.message
      ...
      prize_handleDelete(index,row){
        this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          if(row.id ===''||row.id === null|| row.id===undefined){
            ...
            this.$axios.post('/slyder_adventures_prize/delete',{'id':row.id}).then((response) =>{
              if(response.data.status === 200){
                this.$axios.post('/slyder_adventures_prize/index').then((response)=>{
                  this.prize_tableData = response.data.data
                })
      ...
      Record_handleEdit(index,row){
        if(row.id ===''||row.id === null|| row.id===undefined){
          this.$axios.post('/slyder_adventures_record_fake/save',{'content':row.content}).then((response)=>{
            if(response.data.status === 200){
              this.$message({
                type: 'success',
                message: response.data.message
              });
              this.$axios.post('/slyder_adventures_record_fake/index').then((response)=>{
                if(response.data.status === 200){
          ...
          this.$axios.post('/slyder_adventures_record_fake/update',{'content':row.content,id:row.id}).then((response)=>{
            if(response.data.status === 200){
              this.$message({
                type: 'success',
                message: response.data.message
      ...
      Record_handleDelete(index,row){
        if(row.id ===''||row.id === null|| row.id===undefined){
          ...
          this.$axios.post('/slyder_adventures_record_fake/delete',{id:row.id}).then((response)=>{
            if(response.data.status === 200){
              this.$message({
                type: 'success',
                message: response.data.message
      ...
      addRecord(){
        if(this.RotaryId === ''||this.RotaryId === undefined ||this.RotaryId === null){
          ...
          if(this.Record_tableData.length > 4){
            //this.record = true
          }else {
            let datas = {}
      ...
      getDatas(){
        ...
        //奖品类型
        this.$axios.post('/slyder_adventures/getPrizeType').then((response)=>{
          if(response.data.status === 200){
        ...
        this.$axios.post('/slyder_adventures/read').then((response)=>{
      ...
      submitForm(formName){
        this.$refs[formName].validate((valid) => {
          ...
          if (valid) {
            this.$axios.post('/slyder_adventures/update',sendData).then((response)=>{
             if(response.data.status === 200){
               this.$message({
                 type: 'success',
                 message: response.data.message
