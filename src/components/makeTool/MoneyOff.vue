<template>
  <div class="addVoucher">
    ...
    <section class="addVoucher-main">
      <div class="img-left">
        <img src="../../assets/images/mj.jpg" alt="">
      </div>
      <div class="addVoucher-content">
        <el-form ref="form" :model="form"  :rules="rules" :label-position="labelPosition" label="right" label-width="120px">
          ...
          <el-form-item label="设置" prop="amount">
            <span>满减额度</span>
            <el-input style="width: 60px" v-model="form.amount" @blur="upCode"></el-input>
            <span>元,单笔满</span>
  ...
  export default {
    data () {
      return {
        ...
        rules:{
          name:[
            { required: true, message: '请输入满减券名称', trigger: 'blur' },
            { min: 1, max: 6, message: '长度在 1 到 6 个字符', trigger: 'blur' }
          ],
          amount:[
            { required: true, message: '请输入满减数额', trigger: 'blur' }
          ],
          start_time:[
    ...
    methods: {
      ...
      radios(){
        if(this.form.product_limit_status === '0'){
          ...
          this.isBtn = false
          if(this.full_reduction_coupon_id === ''||this.full_reduction_coupon_id === null||this.full_reduction_coupon_id === undefined ){
            ...
            let sendData = {
              id:this.full_reduction_coupon_id,
              name:this.form.name,
            ...
            this.$axios.post('/full_reduction_coupon/update', sendData).then((response) => {})
          }
        }
        let popupStatus = this.form.popup_status
        if(popupStatus === true){
      ...
      addCommods(){
        if(this.full_reduction_coupon_id === ''||this.full_reduction_coupon_id === null||this.full_reduction_coupon_id === undefined ){
          ...
          this.isShow = true
          this.$axios.post('/goods/indexDiscountCoupon',{full_reduction_coupon_id:this.full_reduction_coupon_id}).then((response) =>{
      ...
      addData(index,row){
        this.$message({
          ...
          type: 'success'});
        let sendData = {
          full_reduction_coupon_id:this.full_reduction_coupon_id,
          goods_type:row.goods_type,
          goods_id:row.goods_id
        }
        this.$axios.post('/full_reduction_coupon_limit_goods/save',sendData).then((response)=>{
          if(response.data.status === 200){
            this.$axios.post('/goods/indexFullReductionCoupon',{full_reduction_coupon_id:this.full_reduction_coupon_id}).then((response) =>{
              this.userListData = response.data.data.data
              this.sum1 = response.data.data.total
            })
            this.$axios.post('/full_reduction_coupon_limit_goods/index',{full_reduction_coupon_id:this.full_reduction_coupon_id}).then((response)=>{
      ...
      assignhandleDelete(index, row){
        ...
        this.$axios.post('/full_reduction_coupon_limit_goods/delete',{id:row.id}).then((response)=>{
          this.$axios.post('/full_reduction_coupon_limit_goods/index',{full_reduction_coupon_id:this.full_reduction_coupon_id,page:this.currentPage}).then((response)=>{
            this.assignTable = response.data.data.data
            this.assignSum = response.data.data.total
          })
        })
      ...
      submitForm(formName){
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$axios.post('/full_reduction_coupon/save',sendData).then((response)=>{
              if(response.data.status === 200){
                this.$message({
                  showClose: true,
                  message: response.data.message,
                  type: 'success'});
                this.full_reduction_coupon_id = response.data.data.full_reduction_coupon_id
              }else {
                this.$message({
                  showClose: true,
                  message: response.data.message,
