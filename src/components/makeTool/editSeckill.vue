        ...
        <div class="buyBambino" style="width: 30%;height: 400px">
          ...
          <div class="buyBambino-main" style="margin-top: 20px">
            <el-form ref="ruleForm" :model="ruleForm" label-width="120px">
              ...
              <el-form-item>
                <el-button type="primary"@click="NoSubmitForm('ruleForm')">保存</el-button>
...
<script>
  ...
  export default {
    ...
    methods:{
      ...
      submitForm(formName){
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$axios.post('/seckill/update',sendData).then((response)=>{
      ...
      //有规格推荐状态设置
     /* rideChang(){
        return this.ruleForm1.recommend
      },*/
      //分页
      ...
      handleCurrentChanges1(val){
        this.$axios.post('/seckill_goods/index',{seckill_id:this.$route.query.data,page:val}).then((response)=>{
          this.commodityData = response.data.data.data
          this.sum = response.data.data.total
        })
      }
    ...
    created(){
      this.$axios.post('/seckill/read',{id:this.$route.query.data}).then((response)=>{
        this.form = response.data.data
        this.seckill_id = response.data.data.id
        this.$axios.post('/seckill_goods/index',{seckill_id:response.data.data.id}).then((response)=>{
          this.commodityData = response.data.data.data
          this.sum = response.data.data.total
        })
      })
