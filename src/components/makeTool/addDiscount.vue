    ...
    <div class="listPro">
      ...
      <div style="margin: 20px auto;">
        <el-pagination
          ...
          :page-size="pagesize"
          @size-change="handleSizeChange"
          @current-change="assignhandleCurrentChanges"
          :total="assignSum">
        </el-pagination>
      </div>
    </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      submitForm(formName){
        this.$refs[formName].validate((valid) => {
          if (valid) {
            ...
            this.$axios.post('/discount_coupon/save',sendData).then((response)=>{
              if(response.data.status === 200){
                this.$message({
                  showClose: true,
                  message: '添加成功！',
                  type: 'success'});
                return this.discount_coupon_id = response.data.data.discount_coupon_id
              }else {
                this.$message({
                  showClose: true,
                  message: '添加失败！',
      ...
      //添加限制商品
      addCommods(){
        if(this.discount_coupon_id === ''||this.discount_coupon_id === null||this.discount_coupon_id === undefined ){
          this.$message({
            showClose: true,
            message: '请先设置保存折扣规则！',
            type: 'error'});
          ...
          this.isShow = true
          this.$axios.post('/goods/indexDiscountCoupon',{discount_coupon_id:this.discount_coupon_id}).then((response) =>{
      ...
      addData(index,row){
        ...
        this.$axios.post('/discount_coupon_limit_goods/save',sendData).then((response)=>{
          if(response.data.status === 200){
            ...
            this.$axios.post('/discount_coupon_limit_goods/index',{discount_coupon_id:this.discount_coupon_id}).then((response)=>{
      ...
      assignhandleDelete(index, row){
        ...
        this.$axios.post('/discount_coupon_limit_goods/delete',{id:row.id}).then((response)=>{
          //限制商品
          this.$axios.post('/discount_coupon_limit_goods/index',{discount_coupon_id:this.discount_coupon_id,page:this.currentPage}).then((response) =>{
            this.assignTable = response.data.data.data
            this.assignSum = response.data.data.total
          })
        })
      ...
      handleCurrentChanges(val){
        ...
        this.$axios.post('/goods/indexDiscountCoupon',{discount_coupon_id:this.discount_coupon_id,page:val}).then((response) =>{
          this.userListData = response.data.data.data
          this.sum1 = response.data.data.total
        })
      },
      //限制商品分页
      assignhandleCurrentChanges(val){
        this.currentPage = val;
        this.$axios.post('/discount_coupon_limit_goods/index',{discount_coupon_id:this.discount_coupon_id,page:val}).then((response)=>{
          this.assignTable = response.data.data.data
          this.assignSum = response.data.data.total
        })
      },
