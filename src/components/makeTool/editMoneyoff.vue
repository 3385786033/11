      ...
      <div class="addVoucher-content">
        <el-form ref="form" :model="form"  :rules="rules" :label-position="labelPosition" label="right" label-width="120px">
          ...
          <el-form-item label="设置" prop="amount">
            <span>满减额度</span>
            <el-input style="width: 80px" v-model="form.amount" @blur="upCode"></el-input>
            <span>元,单笔满</span>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      addCommods() {
        this.isShow = true
        this.$axios.post('/goods/indexFullReductionCoupon',{full_reduction_coupon_id:this.$route.query.data}).then((response)=>{
      ...
      addData(index,row){
        this.$message({
          ...
          type: 'success'});
        let sendData = {
          full_reduction_coupon_id:this.$route.query.data,
          goods_type:row.goods_type,
          goods_id:row.goods_id
        }
        this.$axios.post('/full_reduction_coupon_limit_goods/save',sendData).then((response)=>{
          if(response.data.status === 200){
            this.$axios.post('full_reduction_coupon_id',{full_reduction_coupon_id:this.$route.query.data}).then((response) =>{
              this.userListData = response.data.data.data
              this.sum1 = response.data.data.total
            })
            this.$axios.post('/full_reduction_coupon_limit_goods/index',{full_reduction_coupon_id:this.$route.query.data}).then((response)=>{
      ...
      handleCurrentChanges(val) {
        this.$axios.post('/goods/indexFullReductionCoupon',{full_reduction_coupon_id:this.$route.query.data,page:val}).then((response) =>{
          this.userListData = response.data.data.data
          this.sum = response.data.data.total
        })
      },
      handleCurrentChanges1(val) {
        this.$axios.post('/full_reduction_coupon_limit_goods/index',{full_reduction_coupon_id:this.$route.query.data,page:val}).then((response) =>{
          this.assignTable = response.data.data.data
          this.assignSum = response.data.data.total
        })
      },
      ...
      assignhandleDelete(index, row){
        ...
        this.$axios.post('/full_reduction_coupon_limit_goods/delete',{id:row.id}).then((response)=>{
          //限制商品
          this.$axios.post('/full_reduction_coupon_limit_goods/index',{full_reduction_coupon_id:this.$route.query.data,page:this.currentPage}).then((response) =>{
            this.assignTable = response.data.data.data
            this.assignSum = response.data.data.total
          })
        })
    ...
    created(){
      this.$axios.post('/full_reduction_coupon/read',{'id':this.$route.query.data}).then((response)=>{
