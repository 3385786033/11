  ...
  <div class="eGfrenzy">
    <header>
      ...
      <div class="head-Right">
        <button type="button" class="btn btn-primary" @click="seteGfrenzy">设置大转盘</button>
...
<script>
  export default {
    ...
    methods:{
      ...
      sendSortValue(){
        this.$axios.post('/slyder_adventures_record/index',{keyword_type:this.sortValues,keyword:this.searchValue}).then((response) =>{
          this.tableData = response.data.data.data
          this.sum = response.data.data.total
        })
      },
      paymentType(){
        this.$axios.post('/slyder_adventures_record/index',{order:this.issueTypeValue}).then((response) =>{
          this.tableData = response.data.data.data
          this.sum = response.data.data.total
        })
      },
      handleDelete(index, row) {
        this.$confirm('此操作将永久删除该数据, 是否继续?', '提示', {
          ...
          this.$axios.post('/slyder_adventures_record/delete',{'id':row.id}).then((response) =>{
            if(response.data.status === 200){
              this.$axios.post('/slyder_adventures_record/index',{page:this.currentPage}).then((response) =>{
      ...
      listDelete(index,row){
        ...
        this.$axios({
          url:'/slyder_adventures_record/deleteBatch',
          method:'post',
          data:{
            'id_arr':arr_id
          }
      ...
      handleEdit(index,row){
        this.$axios.post('/slyder_adventures_record/saveRemark',{id:row.id,remark:row.remark}).then((response)=>{
          if(response.data.status === 200){
            this.$message({
              type: 'success',
              message: response.data.message
      ...
      seteGfrenzy(){
        this.$router.push({name:'setTurntable'})
      },
      //分页
      ...
      handleCurrentChanges(val){
        ...
        this.$axios.post('/slyder_adventures_record/index',{page:val}).then((response) =>{
          if(response.status === 200){
            this.tableData = response.data.data.data
            this.sum = response.data.data.total
          }else {
    ...
    created(){
      this.$axios.post('/slyder_adventures_record/index').then((response)=>{
        if(response.data.status === 200){
          this.tableData = response.data.data.data
          this.sum = response.data.data.total
        }
